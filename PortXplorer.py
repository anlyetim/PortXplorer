...
class PortXplorer:
    ...
    def Hedef_tarama(self, hedef, baslangic_port, bitis_port, istek_sayisi=100):
        ...
        try:
            ...
            try:
                ...
                return ["Hata: Geçersiz hedef veya DNS çözülemedi."], ""
    ...
    def Ag_tarama(self, temel_ip, baslangic_port, bitis_port, istek_sayisi=100):
        ...
        if not temel_ip or temel_ip.strip() == "":
            return ["Hata: Temel IP boş olamaz."], temel_ip
...
class UygulamaGUI:
    def __init__(self, root):  
        ...
        tk.Label(sol_cerceve, text="Hedef (IP veya Website):", anchor=tk.W).pack(anchor=tk.W, pady=5)
        self.hedef_girdi = tk.Entry(sol_cerceve)
        self.hedef_girdi.insert(0, self.temel_ip)
        self.hedef_girdi.pack(anchor=tk.W)

        tk.Label(sol_cerceve, text="Başlangıç Portu:", anchor=tk.W).pack(anchor=tk.W, pady=5)
        ...
        tk.Label(sol_cerceve, text="Bitiş Portu:", anchor=tk.W).pack(anchor=tk.W, pady=5)
        ...
        tk.Checkbutton(sol_cerceve, text="Sadece Kendi IP’mi Tara", variable=self.kendi_ip_tarama_degiskeni, anchor=tk.W).pack(anchor=tk.W, pady=5)

        self.ag_tarama_degiskeni = tk.BooleanVar()
        tk.Checkbutton(sol_cerceve, text="Yerel Ağı Tara", variable=self.ag_tarama_degiskeni, anchor=tk.W).pack(anchor=tk.W, pady=5)

        self.tarama_butonu = tk.Button(sol_cerceve, text="Taramayı Başlat", command=self.Taramayi_baslat)
        ...
        self.durum = tk.Label(root, text=f"Kendi IP’niz: {self.yerel_ip}", bd=1, relief=tk.SUNKEN, anchor=tk.W, justify=tk.LEFT)
    ...
    def Taramayi_baslat(self):
        ...
        self.durum.config(text="Tarama yapılıyor...", justify=tk.LEFT)
        ...
        try:
            ...
            self.sonuc_metni.insert(tk.END, "Hata: Port numaraları geçerli bir sayı olmalı!\n", "left")
        ...
        def tarama_isi():
            ...
            self.sonuc_metni.delete(1.0, tk.END)
            if sonuclar and isinstance(sonuclar, list) and "Hata" not in sonuclar[0]:
                self.sonuc_metni.insert(tk.END, "Aktif Portlar:\n", "left")
                for host in sonuclar:  
                    self.sonuc_metni.insert(tk.END, f"{host}\n", "left")
            else:
                self.sonuc_metni.insert(tk.END, sonuclar[0] if sonuclar else "Hiçbir aktif port bulunamadı.\n", "left")
            self.sonuc_metni.insert(tk.END, f"\nTarama {gecen_zaman:.2f} saniyede tamamlandı.", "left")

            self.tarama_butonu.config(state="normal")
            self.durum.config(text=f"Tarama tamamlandı (Tarama IP: {taranan_ip})", justify=tk.LEFT)
